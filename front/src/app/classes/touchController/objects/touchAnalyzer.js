...
export default {
    shouldPlayerStop() {
        return this.shouldPlayerRunLeft() === false
            && this.shouldPlayerRunRight() === false
    },

    shouldPlayerJump() {
        return touchController.was_touch_detected === true
            && touchController.is_pressing === false
    },

    shouldPlayerRunLeft: () => touchController.was_touch_detected === true
        && isPressingOrJustUnpressed(touchController)
        && touchController.position.x - widthToRun > touchController.moving_position.x,

    shouldPlayerRunRight: () => touchController.was_touch_detected === true
        && isPressingOrJustUnpressed(touchController)
        && touchController.position.x + widthToRun < touchController.moving_position.x,
}